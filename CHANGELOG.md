...
## HTPG 九类特征提取工程化（2026-09-20，不改变程序版本）

- 发布准备：按用户后续 `push` 请求，将特征提取与分布分析代码、测试、公开参考词典和不含明文的汇总纳入当前 `cursor/research-review-docs` 分支；不上传原始口令及本地逐条特征，不直接改动 main。提交前重新运行 62 项测试全部通过（8.202 秒）。此前“不推送”描述指实现当时的操作边界。
- 修改：Codex。按用户要求将原论文 III-D 特征提取做成独立代码：`ZipfGuard/core/htpg_features.py`、保真频次行解析器 `core/counted_corpus.py`、流式入口 `experiments/extract_htpg_features.py`。这一步不划分头尾、不计算 IGR、不生成建议、不做攻击、不替换 M1–M4，不修改网页、正式 reports、服务器或远端仓库。
- 九类特征明确对应 LSD 连续段、长度、大写、日期、键盘、符号位置、边界孤立小写、情感词和姓氏。原文未公开的日期/键盘规则采用已文档化的操作定义；多位置符号保存位置集合，是显式扩展而非冒充原文四分类。Unicode 不规范化；无法解码的记录九项为未知，缺词典时词典特征为未知，均不把未知当 False。
- 新增可重建参考词典：VADER 固定版本筛得 7,174 个情感词，FiveThirtyEight 固定版本 Census 姓氏数据筛得 1,000 项；资源包含来源、许可、筛选规则及 SHA-256。明确不是作者原始词表，没有复现其未公开的中文姓氏列表。使用 Aho–Corasick 子串匹配并记录短词误命中边界。
- 程序生成逐条特征 gzip、记录数/频次双口径汇总、中文报告和输入/代码/词典校验清单；保留空格和控制字节，不复制或输出明文。逐条特征仍可关联原语料，故 `local_artifacts/` 及特征 gzip 加入 Git 忽略；失败留下 INCOMPLETE，禁止覆盖旧结果，不把前缀测试冒充全量。
- 新增 32 项特征/输入/流水线/结果核验测试，包括字典搜索与朴素匹配对照、频次守恒、未知语义、隐私、确定性和篡改检测；连同现有测试共 62 项通过（最终实测 15.584 秒）。操作、公式及复现边界见 `ZipfGuard/docs/HTPG_FEATURES.md`。
- 实际由新程序全量提取用户提供的 Rockyou：14,344,391 行、32,603,388 次计数；排除 1 行空口令（340 次），保留 14,344,390 条非空记录 / 32,603,048 次。14,344,172 条生成九项特征，218 条非 UTF-8 记录 / 243 次记未知；1,008 行首尾空白、9 行控制字节保真。输入 SHA-256 与前次分布分析一致；本次提取器不另做重复口令检查。
- 全量提取耗时 130.788 秒，逐条特征 gzip 为 170,539,130 字节；结果在被 Git 忽略的 `ZipfGuard/local_artifacts/htpg_rockyou_20260920/`。新增 `experiments/verify_htpg_features.py` 独立重读全部 14,344,390 条缓存，文件哈希、序号、未知语义、32,603,048 次频次及九项直方图均核对通过；`verification.json` 记录验收。该核验验证数据完整性与统计一致性，不冒充语义识别准确率的独立人工评测。

## Rockyou 带频次数据全量分布分析（2026-09-20，不改变程序版本）

- 按用户要求仅做离线描述统计与分布拟合，新增独立脚本 `ZipfGuard/analysis/fit_counted_corpus.py`，匿名 JSON、中文报告及 PNG 曲线保存在 `ZipfGuard/analysis/rockyou_20260920/`。不修改网页分析器、M1–M4、正式 reports/misc，不运行攻击、不推送或部署，不复制原始口令数据。
- 全量读取 14,344,391 行、32,603,388 次计数；排除 1 行空口令（340 次），得到 14,344,390 种非空口令和 32,603,048 次计数。内存中的 128 位 BLAKE2b 去重检查未发现重复摘要；保留 218 行非 UTF-8 字节、1,008 行首尾空白及 9 行控制字节，不做可能合并不同口令的字符替换或 strip。只导出汇总与文件级 SHA-256，不导出明文或逐口令摘要。
- HTPG 风格的 count>3 对数线性回归：746,289 种，C=829712.5775、alpha=0.9137450、log-R²=0.9963235；原始次数坐标的曲率切点 1172.1164，向下取整头部覆盖 3,900,288 次（11.9630%）。接近原论文 0.914/1171，但数据版本、清理口径不同；排名第一预测偏高，不能把高 R² 当成所有排名都准确。
- 比较离散 Zipf、有限支持 CDF 幂律及有限支持拉伸指数 CDF：全部种类参与共同 14,096 个排名箱的多项式似然拟合；逐排名全量计算误差，无 top-k 重归一化。最大累计偏差分别为 8.2478、7.2007、7.2007 个百分点；拉伸指数 t 接近零，退化为 CDF 幂律，并未得到实质提升。比较的是明确的有限支持参数化，不声称复现 Hou/Wang 原模型或完整检验。
- 记录频次过滤敏感性、26,384 箱细化核对、经验集中度、归一化/分箱概率/曲率自检及图形检查。仅出现一次的种类占 82.8521%，计数占 36.4525%；本次为样本内描述分析，无独立测试、无 KS p 值、无已通过拟合优度检验或真实攻击成功率的结论。

## ZipfGuard M1–M4 可复现实验闭环（2026-09-18，不改变程序版本）

- 修改：Codex。在推送前先同步远端 `main`（基线 `149b1bc`），将本地独立开发的
  ZipfGuard M1–M4 合入总仓库的 `ZipfGuard/` 子目录；保留总仓库其他项目、队友提交
  和根目录变更历史，没有把独立仓库错误地铺到总仓库根目录。
- **M1 统一合成数据**：新增确定性合成用户数据集与固定
  `train/validation/test=60/20/20` 划分；分布、策略和攻击实验共享同一数据集 ID，
  固定种子可复现，聚合结果与用户总数一致，避免模块各用一套样本。
- **M2 攻击基线**：新增统一排名接口和训练集频次、公开合成字典、字符 n-gram 三类
  攻击器；严格执行 train 拟合、validation 选参、test 评价，输出
  `cracked@100/1K/10K`、候选覆盖率和逐点 95% Wilson 区间。固定预算不会因候选空间
  较小而被静默截断，并增加“修改 test 不改变攻击排名”的泄漏回归测试。
- **M3 用户响应与自适应攻击**：被规则拒绝的用户不再从样本中删除，而是按“可预测
  修补→公开随机短语”机制重新选择口令；保留全部 `user_id` 和原数据划分，记录初始
  接受率、完成率、修改率、尝试次数和修改前后分布。同时报告冻结攻击与按响应后
  train 重新训练的自适应攻击，汇总适应增益和最坏攻击者风险。
- **M4 策略搜索**：从最小长度、字符类别、年份后缀、键盘走查和常见词根动作中枚举
  26 个唯一单规则/双规则候选；validation 阶段以最坏自适应风险和总成本为双目标，
  将修改率、尝试次数、长度增长和规则复杂度纳入约束，构造 Pareto 前沿。只有选定的
  低摩擦、均衡、高防护三档进入最终 test 评价；行为等价候选复用攻击计算。
- **合并队友空样本修复**：继续保留“空样本命中率为未知、缺失预算不默认零风险”的
  指标语义；原先“全拒绝后不可评估”的策略回归测试升级为 M3 语义，验证初始全部
  被拒用户仍完整保留、重选后完成率为 100%，冻结/自适应攻击均以完整 test 队列为分母。
- 流水线、CLI、Markdown/JSON 导出、Streamlit 与零依赖本地网页均已接入 M2–M4；
  `README.md`、`docs/DECISIONS.md`、`docs/PROJECT_STATUS.md` 已同步协议、验收结果与
  合成数据边界。新增攻击器、用户响应、策略搜索和数据泄漏专项测试；远端集成工作树
  全量 30 项测试通过。
- 按用户要求，本次提交和推送不新增、不改写、不删除 `ZipfGuard/reports/` 下的报告；
  本地生成的 `reports/*.json` 与 `reports/*.md` 继续由 ZipfGuard 自身 `.gitignore` 排除。

## ZipfGuard 全拒绝策略的空样本修正（2026-09-18，不改变程序版本）

- 修改：Codex。修正 `ZipfGuard/policy/engine.py` 与 `core/metrics.py`：全部样本被拒绝时标记 `not_evaluable`，命中率、安全收益及相应风险差值返回null，而非0命中/100%收益；增加总数、接受、拒绝及评估样本数。保留真实零命中与未知的区别，缺失预算结果也不再默认零风险。
- 推荐器只使用有评估样本和有限收益的候选；无可评估候选时返回空推荐，无正收益候选时不输出“高防护”。两种网页和Markdown报告显示“无法评估”、样本计数及解释，将展示中的用户成本明确为样本拒绝率。未改变规则、分布模型、旧分类器、正式reports/misc或AI接入。
- 新增9项空样本、正常/部分接受、推荐过滤、缺失指标及JSON/Markdown回归测试，全部通过。全套19项中18通过，原有模型目录检查仍因本机缺少外部权重目录失败，未修改该无关测试。执行网页JavaScript渲染检查及本地HTTP `/api/demo`、`/api/rockyou` 验证，确认null传播、计数和推荐过滤正确。
- 用现有聚合输入重新生成ZipfGuard内部的demo与rockyou JSON/Markdown报告，清除旧的虚高收益；本地8765网页已启动为新代码。按用户后续要求将本次修复纳入提交和推送范围，保留其他未提交研究文档；未改变根目录正式reports数据，不部署远端服务器。

## 拉取并审阅 ZipfGuard 新方案（2026-09-16，不改变原程序版本）

- 整理：Codex。按用户要求将本地 main 从 `f0aa2ab` 快进拉取至 `44d2652`（作者 zouyunkaicauc，新增独立 ZipfGuard 目录、32个文件）；保留已有未提交文档和CHANGELOG。不修改测量内核、正式 reports/misc，不部署、不推送。
- 对照用户提供的HTPG原始PDF和DP-HTPG方案；仓库方案与附件逐字节一致。明确原论文是IEEE TIFS 2022期刊论文，69%为其模拟修改及既定猜测协议下的平均结果，80.23%为记住原口令条件下的增强口令回忆率，不是分类准确率或本项目结果。
- 实际运行ZipfGuard的10项Python单元测试：9通过，1因本机缺少测试所硬性要求的外部模型目录失败；未下载或运行神经模型。离线重跑两种聚合输入，确认模型选择不同但四项策略结果完全一致，因为策略评估固定使用独立合成语法。
- 复现 `diversified-phrase` 接受率0、报告security_gain=1且获“高防护”推荐；这是空测试群体的指标/推荐问题，不能算防护收益。确认优化器只评估四个预设策略，PassLLM.generate仍抛NotImplementedError，原始曲率/IGR策略生成、完整约束搜索和神经红队尚未实现。
- 独立运行JS合成红队：默认样本9000、测试1800；预算100时，原始/统一追加后缀的自适应猜测命中率约78.33%/78.17%，冻结猜测器对后缀组为0。仅为合成演示，说明需要评估知晓规则的攻击者，不代表真实人群收益；该JS流程尚未接入Python网页策略流水线。本次仅检查和记录，未修复这些问题。

## 口令生命周期一致性实验计划（2026-09-15，不改变程序版本）

- 修正文档面向团队的执行环境说明：不限定Mac，改为组员各自电脑或团队授权测试环境；此修订不代表已获准操作任何服务器。
- 按用户要求将上述计划压缩为简短行动清单：目标、四项测试、本地执行方式和交付物；删除长篇背景与指标展开。仅改文档，实验仍未启动。
- 整理：Codex。新增 `docs/research_review_20260915/04_口令生命周期一致性小实验计划.md`，将讨论收敛为注册、修改、重置与登录的跨流程一致性可行性实验，明确三天左右的首轮预算、正负对照、证据字段、数学指标、论文方法及继续/停止标准。
- 明确该文档是尚未实施的计划；只修改文档和本记录，不修改程序、正式 reports/misc、版本、服务器或Git远端，不启动测量。区分已知缺陷对照、真实发现与拟议创新，限定首轮Mac本地受控环境，不将访客页面反馈当作服务端认证事实。

## 千站三阶段最终结果与源码重新审阅（2026-09-15，不改变程序版本）

- 新增 `docs/research_review_20260915/` 三份 Markdown：原始 Frank Li 两篇论文对照、千站三阶段原始结果与结构审计、相关论文与可验证拓展方向；保留2026-09-14旧文档。整理：Codex；仅新增文档和本记录，不修改测量内核、正式 reports/misc、部署或Git远端。
- 从服务器读取并校验原始预检/分类/政策JSONL与运行元数据，文档保留SHA-256及去重口径。第三阶段31/31结束，耗时2504秒；23站保留密码步骤，10站650条正式探针，6站有长度/组合结果，2站通过当前完整门控。明确650不包含未落入正式数组的前置探针，2个complete不等于独立验证正确，21个partial含13个无正式主动政策的站。
- 逐站拆解31站结果及6个实质政策产出：4站允许项缺失；荣耀云与Linode上限直接来自HTML；Stripe一字符在异步复验分支被标接受后负对照失败；ipinfo180条同源拒绝后900秒超时。保留因果限制，不将程序标签当成网站服务器政策或将未确认的CSS根因写成事实。
- 离线运行现有 `tests.test_policy_quality`，10项通过；用真实函数验证4个合成反例：跨表单A/R仍complete、no_web_signup计入口、无states的verification_then_password计密码到达、空policy只见字段即partial。未启动浏览器或新实站测试，未修复上述问题。
- 重新核对论文原文、方法与工具，新增WWW 2024认证页面分类基线、USENIX 2015猜测模型与SOUPS 2026更新流程等相关依据；区分会议类别、论文明确局限与拟议创新，记录来源之间的标注样本口径差异。研究建议聚焦可靠判定、属性推断与可测性偏差。

## 研究审阅文档（2026-09-14，不改变程序版本）

- 第三阶段运行记录（2026-09-14）：按用户要求在服务器启动 31 站口令政策测量，筛选第二阶段最新 signup 记录中无 error、stop_reason=password_step_reached 且 states 实际含 password 字段的目标。使用 main@f0aa2ab 独立代码快照，避免网页定时同步改变运行代码；仅 signup、auto inline、无头、并发 2、单站 900 秒、不自动复测，显式关闭 full-form。结果独立保存于服务器 `.cache/target-corpora/tranco-1000/policy-stage3-20260914/`，含候选清单、源数据哈希、运行元数据、日志和 policy.jsonl；不覆盖第二阶段或正式 reports。此项只记录已启动，不承诺31站均可测或均完整；已知推断/质量门控问题尚未修改，完成状态仍需按证据审查。
- 按用户要求新增 `docs/research_review_20260914/` 下三份 Markdown：Frank Li 两篇 2023 论文与项目的方法对照、主分支结构缺陷与实验数据审计、相关顶会论文与一个月研究路线。整理：Codex；只修改文档。
- 审阅基线为 `main@f0aa2ab`，另核查 `cjx/b-series-local-20260914@24e9c09`；明确 B 系列保留能力不等于当前主分支已集成。纠正将长度二分当作相对 CCS 2023 原论文创新的说法。
- 核实服务器最终快照：1000 目标预检通过 413、失败 587；分类原始 828 行，按站点/入口去重后 826 个任务完成。该批第三阶段口令政策测量尚未执行，不将分类任务数冒充政策完成数。保留 303 站历史基线，明确 complete 不等于独立验证正确。
- 记录登录任务先走注册导航、固定负对照、HTML 上限证据升级、身份项未绑定、质量门控缺少同表单探针绑定等问题及建议；本次不实施这些修复、不修改 reports/misc 权威数据、不启动新扫描、不部署或推送。
- 研究建议收敛为低交互口令政策可靠推断、反馈一致性与可测性偏差；为每条研究路线标明论文方法/工具、公式、代码落点、实验口径和授权边界，避免将拟议创新与未完成实验写成既有成果。

## v4 进行中（2026-08-15）

### 67. 浏览器看门狗恢复、批量无头模式与真实完整回归（2026-09-12）

- [x] 单站看门狗超时后只恢复本轮进程写入且包含主动探针轨迹的新鲜阶段检查点，输出
      `partial_timeout` 和严格质量原因；旧缓存、空政策和纯静态提示不会被冒充为测量。
- [x] 中间阶段检查点同步写入脱敏 `_probe_evidence`；浏览器窗口/会话意外终止时，批量包装器
      自动重建一次会话，并记录 `browser_session_restarts`，不再让整批任务连续空转。
- [x] 批量 CLI 新增 `--headless` 与 `--inline-accept-quiet-seconds`。只有同页一字符负对照
      已明确被拒后，候选字段持续干净 2--8 秒才允许结束观察；pending、门控软错误和原生
      invalid 状态均不会走快速路径。
- [x] 删除每轮探针首尾重复的鼠标移出操作，保留 reset 后和填充后的真实 blur；Unicode
      “任一字符可接受”在首条正证据后停止，并新增 Emoji 的真实 setter/事件兜底探针。
- [x] 泄露口令判断记录样本量与决策依据：任一符合其他政策的泄露口令被接受即可证伪；
      连续拒绝样本预算默认 5、可配 3--20；没有合格样本时返回未知而不是 False。
- [x] 严格验收不再只检查字段名：限制项必须是正确的布尔/非负整数，允许项必须是布尔值，
      `None` 或错误类型不能计为完整。
- [x] 主动结果新增脱敏 `_measurement_seconds` / `_probe_count` 吞吐遥测，便于千站分片估算
      剩余时间；遥测不参与政策结论，也不保存候选口令。
- [x] LinkedIn 真实无头回归在不提交表单、不发送验证码、不创建账号的边界内完成全部政策
      维度：长度下限 6、128 位内未观察到上限、37 条脱敏探针；严格状态 `complete`。
      同机同站总耗时由第一轮 343 秒降至 247 秒，且 Emoji 从旧版“未测即 False”改为实测接受。

### 68. 授权范围清单与浏览器进程树清理（2026-09-12）

- [x] 批量 CLI 新增 `--authorization-manifest`，支持精确主机/窄通配符、过期时间和
      SHA-256 范围指纹；不在所有者授权范围内的目标会在启动前过滤，结果不保存清单内容。
- [x] 单站超时看门狗在终止 worker 前枚举并清理其浏览器子进程，避免千站分片长期运行时
      Edge/Chrome 残留耗尽内存；psutil 与 pyvirtualdisplay 作为服务器安装依赖显式声明。
- [x] 安静窗口要求显式 `valid=True`，缺失字段不再被误判为候选可接受；新增范围、进程清理
      与边界状态回归测试。
- [x] 授权范围保留清单中显式的 `www` 标签，避免 `*.www.example` 被规范化后意外扩大
      为整个 `example` 域；普通根域仍兼容其 `www` 别名。
- [x] 批量调度按主机串行化同站 signup/login 任务，不同主机继续并行，降低多入口重复
...
- [x] 新增 Tranco 快照与中文补充清单合并工具，输出永久列表 ID、来源、排名和去重元数据；
      当前本地候选语料包含 100024 个不同域名。
- [x] 新增低成本 HTTP 预筛：限制公开 IP、跳转协议、响应大小、超时和每站页面数；沿
      主页→登录/账户→注册入口浅爬，生成按优先级排序的多个浏览器候选。
- [x] 修复产品 CTA、重定向查询参数、产品/保修/活动注册等假入口；浏览器会依次重新
      验证最多 8 个观察到的同站候选，不会把生成的常见路径批量当成可信提示。
- [x] 新增 live DOM/HTML5 证据：`minlength`、`maxlength`、`pattern`、原生 validity、
      ARIA/关联说明及中英文规则解析；与主动策略分层保存，不用提示规则冒充实测。
- [x] 新增统一严格完整性门和覆盖报告。303 站历史基线仅 5 站严格完整，页面可达、入口、
      口令框、主动测量、对照、长度、限制和允许项均单独统计。
- [x] 批处理修复 Windows 子进程 Queue 丢结果/大结果管道死锁和 GBK 状态字符崩溃；新增
      `complete` 质量续跑、确定性分片、优先级/站点上限、质量 checkpoint。
- [x] Dashboard 明确区分“任务跑完”“部分证据”“完整测量”，展示 DOM/声明证据及缺失
      门槛；旧缓存必须重新通过严格证据门，不能仅凭非零长度直接标记成功。
- [x] 修复英文规则解析把 `minimum number of characters` 错当作数字要求、把混合允许/禁止
      语句全局污染的问题；DOM/声明最短长度和字符类现在只用于优先安排候选，最终结论仍须
      主动接受/拒绝证据。
- [x] 移除随机伪造 Gmail/Foxmail/Yahoo 测量地址；默认使用不可投递保留域，支持显式配置
      研究者自有固定邮箱或 catch-all 域，并仅在结果中记录脱敏身份模式元数据。
- [x] 新增千站路线文档和相应单元/集成回归，安全边界仍是不绕过 CAPTCHA、不发验证码、
      默认不提交或创建账号。

### 65. 长度与字符组成的分段 OR 政策（2026-09-11）

- [x] 新增共享 `adaptive-conditional-v1` 规划器，复用字符组成阶段已明确拒绝的单类
      候选，在实测最短长度建立严格分支，再主动寻找更长口令下的组成放宽。
- [x] 使用指数扩张快速找到“拒绝/接受”区间，再通过二分定位精确阈值；最终阈值
      由第二个同结构候选确认，降低异步反馈或偶然文本规则造成的误报。
- [x] 输出结构化 `OR` 两分支：短口令分支包含最短长度、最少类别和必需类，长口令
      分支包含放宽阈值与已接受字符子集；未检出与证据不足保持独立状态。
- [x] inline 与 full-form 均接入 16 次额外预算和阶段前后锚点控制；条件探测自身证据
      不足会单独标记，不撤销已完成的基础长度与组成结论。
- [x] Dashboard 增加“长度×组成条件政策”摘要和脱敏阈值搜索表，不再把检测到的
      GitHub 式政策错误显示为全局字符组成要求。
- [x] 新增 6 项 8/15 分段政策、未观察到放宽、三态中止、不适用及 inline/full-form
      共享规划器测试；完整 266 项回归测试通过。

### 64. 自适应字符组成推断第二阶段（2026-09-11）

- [x] 新增共享 `adaptive-composition-v1` 规划器，在已确认可接受口令的固定长度下，
      按小写、大写、数字、符号的子集格逐层实验；不再依赖固定顺序的单类突变。
- [x] 完整探测首个可接受层级，分别输出最少字符类数、全部已接受最小组合和这些
      组合的交集，从而区分对称“任意 N-of-4”政策与非对称“指定字符类必需”政策。
- [x] 对固定必需类保持长度和其余类别不变，用二分决策推断最低数量；候选构造保留
      已接受锚点中的类别计数，并遵守禁用符号、字母开头和多词等已知结构条件。
- [x] full-form 符号测试改为等长替换并加入非符号对照，避免把最大长度拒绝或丢失
      原字符类造成的拒绝误判成“禁止特殊符号”；自适应探测前重新验证缓存锚点。
- [x] inline 与 full-form 共用同一规划器和 24 次探测预算；无密码专属反馈、预算
      耗尽或无法控制变量时保留 `inconclusive` / `partial`，不会强制写成零要求。
- [x] 兼容写回旧版 `r_*_min` 与 `r_cmbN4` 字段，同时新增 `_adaptive_composition`
      完整证据；Dashboard 可展开查看字符组合、最低数量、候选结构和决策路径，且
      不保存候选口令原文。
- [x] 新增 10 项字符子集、非对称必需类、数量边界、禁用符号、三态中止、脱敏、
      full-form 集成及等长符号对照测试；完整 260 项回归测试通过。

### 63. 自适应口令长度推断第一阶段（2026-09-11）

- [x] 新增共享 `adaptive-length-v1` 探测规划器，以已确认可接受口令为锚点，
      通过二分决策自适应选择下一个长度；inline 与 full-form 不再维护两套互不一致
      的长度扫描逻辑。
- [x] full-form 最坏约 156 次的线性长度扫描改为受 24 次预算约束的自适应搜索；
      模拟 8--16 位四类字符政策可在少于 16 次探测内精确恢复上下界。
- [x] 长度候选构造会保持已知字符类、最少字符数、N-of-M 组合、字母开头和多词
      结构，尽量让长度成为唯一变量；无法容纳已知组成约束的长度不作为拒绝证据。
- [x] full-form 反馈升级为接受/拒绝/证据不足三态：没有密码专属反馈、浏览器终止或
      重试耗尽不再自动解释成网站拒绝。
- [x] 策略结果增加脱敏自适应证据：探测预算与用量、边界值和置信度、搜索区间、
      下一步决策、相对最坏线性扫描的节省量；Dashboard 支持展开查看决策路径。
- [x] 新增自适应边界、超长最低要求、拒绝复验、无上限、HTML maxlength、三态
      中止、候选脱敏和两条测量链路兼容测试；完整 250 项回归测试及 Python 编译
      检查通过。
