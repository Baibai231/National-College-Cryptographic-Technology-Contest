...
| 提交 | 修复或新增内容 |
|---|---|
| 当前开发版 | 修复 Python 3.12 浏览器驱动兼容；新增 Windows 本地一键展示；历史实测口令策略正确映射到 Dashboard；新增认证语义状态/动作边去重；分类现场口令字段直接交给策略测量器；合法口令搜索覆盖扩展至 64 位且常见边界优先 |
...
安全观察器、评分、API 权限、任务队列、Cookie 横幅和前端展示等关键路径。
