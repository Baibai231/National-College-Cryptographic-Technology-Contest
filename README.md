...
- `adaptive-conditional-v1` 会复用被拒绝的字符子集，以指数扩张加二分搜索主动寻找
  “短口令要求多类、达到更长阈值后单类也可接受”的分段 OR 政策，并复验阈值接受证据。
...
- 对存在字符组成限制的站点，会主动检查更长口令是否触发组成放宽；检测结果输出为
  两个可解释 OR 分支，不再把条件政策错误压缩成全局 AND 约束。
